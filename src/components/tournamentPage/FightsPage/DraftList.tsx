  ...
  return (
    <>
      <td className="text-start border-r border-border px-2 py-1">Команда <strong>{chellendger.name}</strong> вызывает
        ...
        задачу <a className="font-bold hover:text-text-hover" href={`/problems/${problemData?.id}`}>№{problemData?.local_number ?? (problemDataTournament?.find(p => p.id === problemData?.id)?.local_number ?? `(${problemData?.global_number})`)} {problemData?.problem_translations[0].problem_name}</a></td>
      <td
