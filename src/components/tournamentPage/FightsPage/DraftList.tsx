...
import {useGetProblemsByIdQuery, useGetProblemsForTournamentQuery, useGetProblemsQuery} from "@/api/problems/clientApiInterface";
import twclsx from "@/utils/twClassMerge";

export default function DraftList({actionId, tournamentId}: { actionId: number, tournamentId: number }) {
          ...
          <div className="border border-border rounded-2xl overflow-hidden">
              <table className="w-full text-text-main border-collapse border-spacing-0">
                  ...
                  {actionData?.drafts?.calls.map((call, index) => (
                    <tr key={index}>
                      {chellendger && chellendged && <DraftItem call={call} chellendger={chellendger} chellendged={chellendged} tournamentId={tournamentId}/>}
                    </tr>
                  ))}
                  </tbody>
              </table>
...
function DraftItem(
  {call, chellendger, chellendged, tournamentId}: {
    call: CallInterface,
  ...
  const {data: problemDataTournament} = useGetProblemsForTournamentQuery({tournamentId: tournamentId})
  return (
    <>
      <td className="text-start border-r border-border px-2 py-1">Команда <strong>{chellendger.name}</strong> вызывает
        ...
        задачу <a className="font-bold hover:text-text-hover" href={`/problems/${problemData?.id}`}>№{(problemData?.local_number && problemData?.local_number <= 17) ? (problemDataTournament?.find(p => p.id === problemData?.id)?.local_number ?? `(${problemData?.global_number})`) : `${problemData?.global_number}`} {problemData?.problem_translations[0].problem_name}</a></td>
      <td
