...
function FightNavigation() {
    ...
    return (
        <>
            <Tabs.Root className="w-full size-full px-3" defaultValue={tabs[0].value}>
                <Tabs.List className="w-ful border border-border rounded-xl relative z-0 flex flex-row md:flex-row gap-1 overflow-x-auto px-4 py-1 md:py-0 ">
                ...
                {tabs.map((item, idx) => (
                    <Tabs.Panel key={idx}
                        className="w-ful py-2.5 outline-accent-primary relative flex h-fit items-center justify-center -outline-offset-1 focus-visible:rounded-md focus-visible:outline"
                        value={item.value}
                    >
