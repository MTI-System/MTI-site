...
export default function UserResultsTable({ tournamentId }: { tournamentId: number }) {
  ...
  const tableData = useMemo(() => {
    ...
    return resTable.results.map((result) => ({
      name: result.user_third_name
        ? `${result.user_second_name} ${result.user_first_name}. ${result.user_third_name}.`
        : `${result.user_second_name} ${result.user_first_name}.`,
  ...
  return (
    <>
      ...
      {resTable && tableData && (
        <div className="border-border text-text-main overflow-auto rounded-2xl border">
          <table className="w-full">
            <thead>
              <tr>
                {columns.map((column) => (
                  <th key={column.field} className="cursor-pointer px-3 py-2" onClick={() => selectSort(column.field)}>
                    <div className="flex items-center justify-center gap-1">{column.header}</div>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {sortedData.map((member, index) => (
                <tr key={index} className="border-border rounded-2xl border-t">
                  {columns.map((column) => (
                    <td
                      key={column.field}
                      className={`truncate px-3 py-2 ${column.field === "name" ? "text-left" : "text-center"}`}
                    >
                      {member[column.field]}
                    </td>
                  ))}
