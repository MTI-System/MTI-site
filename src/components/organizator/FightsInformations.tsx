...
export default function FightsInformations() {
    ...
    const changeContainer = useCallback((id: number, value: string)=>{
    ...
    return (
        <>
                    ...
                    <Popover.Portal>
                        <Popover.Positioner sideOffset={8} align={"end"}>
                            <Popover.Popup className="origin-[var(--transform-origin)] rounded-lg bg-[canvas] px-6 py-4 text-gray-900 shadow-lg shadow-gray-200 outline outline-1 outline-gray-200 transition-[transform,scale,opacity] data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[starting-style]:scale-90 data-[starting-style]:opacity-0 dark:shadow-none dark:-outline-offset-1 dark:outline-gray-300">
                                <Popover.Title className="text-base font-medium">Добавьте список боев, которые будут играться на турнире!</Popover.Title>
                                <Popover.Description className="text-base text-gray-600">
                                    Каждый добавленный пунки - это один отборочный бой или финал.<br/>
                                    Пример списка:
                        ...
                        <>
                            <div className="flex flex-col justify-between" key={`fights-${container.id}`}>
                                <div className="flex justify-between">
                                    ...
                                    <button className="bg-[#ED0F4E]/20 border hover:bg-[#ED0F4E]/50 border-[#ED0F4E] text-[#ED0F4E] h-[2.5rem] rounded-2xl w-[6rem] mt-2"
                                        onClick={
                                            ()=>setFightsContainers([...fightContainers].filter((f, idx)=> f.id !== container.id))
                                        }
                                    >Удалить</button>
                                </div>
                            </div>
            ...
            <button
                ...
                onClick={()=>{
                    setFightsContainers([...fightContainers, {id: (fightContainers[fightContainers.length - 1]?.id??0) + 1, name: "Название боя (отборочный или финал)"}]);
                }}
            >
                Добавить раздел
            </button>
