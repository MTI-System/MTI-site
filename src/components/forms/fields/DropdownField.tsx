...
className, type, name, onVerification
    ...
    useEffect(()=>{
        // register(()=>{
        //     // const result = onVerification(valueRef.current inst)
        // })
    }, [])
    ...
    return (
        <>
            <Select.Root multiple items={fonts} onValueChange={(value)=>{
                valueRef.current = value
            } }>
            <Select.Trigger className="flex h-10 min-w-36 w-fit items-center justify-between gap-3 rounded-md border border-gray-200 pr-3 pl-3.5 text-base text-gray-900 select-none hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 data-[popup-open]:bg-gray-100 cursor-default">
                <Select.Value/>
