...
export default function FieldTypes(
  {setFieldType}:{setFieldType:(value:availableFields)=>void}
){
  ...
  return (
    <>
      <Select.Root multiple={false} items={types}
                   defaultValue={"text"} onValueChange={(value) => {
        // valueRef.current = value
        setFieldType(value)
      }}>
        <Select.Trigger
          className="flex h-full min-w-36 w-fit items-center justify-between gap-3 rounded-md border border-gray-200 pr-3 pl-3.5 text-base text-gray-900 select-none hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 data-[popup-open]:bg-gray-100 cursor-default">
          <Select.Value/>
          ...
          <Select.Positioner className="outline-none select-none z-10" sideOffset={8}>
            <Select.Popup
              ...
              <Select.ScrollDownArrow
                className="bottom-0 z-[1] flex h-4 w-full cursor-default items-center justify-center rounded-md bg-[canvas] text-center text-xs before:absolute before:left-0 before:h-full before:w-full before:content-[''] bottom-0 data-[side=none]:before:bottom-[-100%]"/>
            </Select.Popup>
          </Select.Positioner>
        </Select.Portal>
      </Select.Root>
