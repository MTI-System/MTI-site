...
export type availableFields = "dropdown" | "text" | "number"
...
export type FieldProperties = DropdownProperties | TextInputProperties | NumberInputProperties
...
export function ConstructorRootProvider({
  ...
  const addField = (fieldName: string, id: number)=>{
    setFields(prev => [...prev, {
      fieldName: fieldName, properties: {fieldType: "text"}, id
    }])
  }

  const setFieldType = (type: availableFields, id: number) => {
    setFields(prev => prev.map((val)=>{
  ...
  return <ConstructorRootContext value={{ getFieldById, fields, addField, setFields, setFieldType, setProperties, changeName}}>{children}</ConstructorRootContext>
}
