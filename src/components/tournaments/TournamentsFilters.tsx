...
export default function TournamentsFilters(
    {
        availableYears,
        availableStates,
    }: { availableYears: number[], availableStates: TournamentState[] }
) {
    ...
    return (
        <>
            <div className="flex items-center pt-3 px-30 w-full justify-between">
...
export function TournamentStateFilter({
    ...
    const optionList: { children: string; value: TournamentState; active: boolean }[] = [
        ...
        ...possibleStates?.map(p=>{
            return {
                children: p === "futured" ? `Запланированные` :
                          p === "registration"? "Регистрация" :
                          p === "processing"? "Идущие" :
                          p === "ended"? "Завершенные" : "Другие",
                value: p,
                active:true
            }
        }),
