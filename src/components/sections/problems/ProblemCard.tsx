...
export function ProblemCardContent({
  ...
  const [editedProblemName, setEditedProblemName] = useState(problem.problem_translations[selectedTrnslation].problem_name)
  const [editedProblemText, setEditedProblemText] = useState(problem.problem_translations[selectedTrnslation].problem_text)
  ...
  useEffect(() => {
    if (editedProblemNumber !== problem.global_number ||
  ...
  return (
    <>
        ...
        <div className={style.titleContainer}>
          {(!startTransition && is_edit_page) && (
            <div className={style.editProblemLineDiv}>
              <Input className={style.problemNumberInput} name="globalNumber" type="number" min={1} max={20}
                     defaultValue={problem.global_number}
                     onChange={(event) => setEditedProblemNumber(Number(event.target.value))}/>
              <Input className={style.problemNameInput} name="firstTranslationName" type="text"
      ...
      <div className={style.textContainer}>
        {(!startTransition && is_edit_page) && (
          <div>
            <textarea className={style.problemTextEditArea} name="firstTranslationName"
      ...
      {(!startTransition && is_edit_page) && (
        <Button className={style.confirmEditButton} disabled={!isEdited}
                onClick={
                  async () => {
                    setIsLoading(true)
                    setIsEdited(false)
