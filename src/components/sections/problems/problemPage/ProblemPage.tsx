...
async function ProblemPage({ problem }: { problem: Problem }) {
  ...
  if (userAuth && userAuth.rights.length !== 0) {
    isModerator = userAuth.rights.map(right => right.right_flag == "MODERATE_PROBLEMS_" + problem.tournament_type).some(x=>x)
  }
  ...
  const listOfMaterials = allMaterials.filter(mat=>(mat.material_type.type_title!=="gif" &&  mat.material_type.type_title!=="video"))
    ...
    <div className={style.pageRoot}>
      <div className={style.problemContainer}>
        ...
        <div className={style.filesContainer}>
          {isModerator && <BlueButton>Добавить материал</BlueButton>}
          {listOfMaterials.map(material=>{
            return (
              <div key={material.id}>
                <a href={FILES_SERVER + material.url}>
                  <img src={FILES_SERVER + material.material_type.logo_path} style={{ width: '10vw', height: '10vw' }}  alt={"pdf"}/>
                  <p>{material.material_name}</p>
                </a>
                {material.material_type.type_title === "pdf" && (<a href={FILES_SERVER + material.url} target='_blank'><BlueButton>Скачать</BlueButton></a>)}
              </div>
            )
          })}
        </div>
