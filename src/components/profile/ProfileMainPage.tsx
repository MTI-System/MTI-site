...
export default function ProfileMainPage({ profileData }: { profileData: User }) {
  ...
  const { data: user, isLoading, refetch } = useGetUserByAuthIdQuery({ id: profileData.user_id })
  return (
    <>
      {!isLoading ? (
        <div className={"text-text-main flex w-full justify-center py-2"}>
          <div className={"bg-bg-alt flex w-full flex-col items-center justify-center rounded-2xl py-5"}>
            <h1 className={"text-center text-3xl font-bold"}>
              Добро пожаловать, {user?.firstName} {user?.secondName} {user?.thirdName} !
            </h1>
            {/*<Instructions/>*/}
            <Accordion.Root className="text-text-main flex w-full max-w-[calc(100vw-8rem)] flex-col justify-center overflow-hidden rounded-xl py-10">
              <Accordion.Item className="">
                <Accordion.Header>
                  <Accordion.Trigger className="group bg-bg-main hover:bg-hover relative flex w-full items-baseline justify-between gap-4 py-2 pr-1 pl-3 text-left font-medium focus-visible:z-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-800">
                    Мой профиль
                ...
                <Accordion.Panel className="h-[var(--accordion-panel-height)] overflow-hidden text-base text-gray-600 transition-[height] ease-out data-[ending-style]:h-0 data-[starting-style]:h-0">
                  ...
                  <div className="p-3">{!user && <p>Ошибка загрузки информации о пользователе</p>}</div>
                </Accordion.Panel>
              </Accordion.Item>
              <Accordion.Item className="">
                <Accordion.Header>
