...
export default function LoginPage(){
    ...
    return (
        <div>
            <input type={"text"} placeholder={"Логин"} onChange={(event)=>{setLoginText(event.target.value)}}  />
            <input type={"password"} placeholder={"Пароль"} onChange={(event)=>{setPassText(event.target.value)}}/>
            <button id={"login_button"} onClick={()=>{
                ...
                // localStorage.setItem("mti_auth_key", );
            }}>{loginButtonState}</button>
        </div>
    )
}
