...
function SubmitButton({
  ...
  return (
    <button
      className="bg-accent-primary/20 hover:bg-accent-primary/50 border-accent-primary text-accent-primary h-[2.5rem] w-50 cursor-pointer rounded-2xl border"
      onClick={() => {
...
function ToastList() {
  ...
  return toasts.map((toast) => (
    ...
    >
      <Toast.Content
        className="overflow-hidden transition-opacity [transition-duration:250ms] data-[behind]:pointer-events-none data-[behind]:opacity-0 data-[expanded]:pointer-events-auto data-[expanded]:opacity-100"
        onMouseDown={(e) => {
          if (e.button === 1) {
            e.preventDefault()
            close(toast.id)
