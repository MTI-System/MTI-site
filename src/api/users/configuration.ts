...
import {emailSocket} from "@/api/auth/SocketConnection";
...
export const defineUsersEndpoints = (
  ...
  verifyEmail: builder.query<string | null, { email: string | null }>({
    ...
    async onCacheEntryAdded(arg, {updateCachedData, cacheDataLoaded, cacheEntryRemoved}) {
      ...
      await cacheDataLoaded
      const unsubscribe = await emailSocket.subscribe((ev) => {
...
// const userToken = Array.from(arr).map((v) => chars[v % chars.length]).join("")
//
//
// const getSocket = () => {
//   return new Promise<WebSocket>((resolve, reject) => {
//     console.log("getSocket", _socket, userToken)
